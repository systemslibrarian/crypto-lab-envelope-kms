 ...
 *
 * This is the SECOND contrast oracle in this repo, and it is deliberate. The
 * first, `contrast/measure.mjs`, works from real screenshot pixels: it shoots
 * the page, shoots it again with every glyph transparent, and reads the surface
 * out of the second shot at the coordinates that changed. That method is
 * immune by construction to the three bugs this file has had to fix over the
 * fleet, because it never reasons about geometry or ancestry at all.
 *
 * It has one hole this file closes: text painted in EXACTLY its background
 * colour changes no pixels when it is hidden, so the pixel differ finds nothing
 * to measure and drops the row silently. The worst possible failure — 1.00:1 —
 * is the one it cannot see. This walk computes the ratio from declared colours
 * instead, so that row is reported rather than dropped. In the other direction,
 * the pixel differ sees things this walk cannot: `body::before` is a fixed,
 * full-viewport grid overlay painted OVER every glyph on the page, and it is
 * nobody's ancestor. Keeping both is what makes the pair complete.
 *
 * The general case this file exists for: axe is not a complete contrast oracle,
 * and two classes of text never reach the `violations` array a gate asserts on.
 *
 *  - text over a surface axe declines to resolve — a gradient, an `rgba()` over
 *    an unknown backdrop, or a `color-mix()`. This page is the extreme case:
 *    `<body>` carries TWO radial gradients and a linear gradient in dark theme
 *    and another linear gradient in light, and axe refuses to compute a ratio
 *    for anything sitting on any of them — 199 nodes of `incomplete` on first
 *    paint alone. On top of that almost every surface is translucent: `--panel`
 *    is `rgba(8,18,25,0.88)`, `.preset-card` and `.prop-card` are
 *    `rgba(7,16,22,0.55)`, `.wrap-step` is `rgba(7,16,22,0.5)`, and the hero
 *    aside is `color-mix(in oklab, var(--accent) 6%, transparent)`. A
 *    violations-only gate measured the contrast of almost nothing on this page.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab has been here
 *    already: `.cl-hero-sub` carried `opacity: 0.85` on top of an
 *    already-dimmed token and measured 3.23:1 in light theme, and the gate of
 *    the day injected `opacity: 1 !important` before scanning, so it could not
 *    have seen it. The declaration is gone and the comment explaining why is
 *    still in `style.css`; the handling below is what keeps the next one
 *    visible.
 *
 ...
 *    boxes are `.hierarchy-scroll` and `.comparison-table` (both
 *    `overflow-x: auto`, and both genuinely overflowing below 900px — the
 *    hierarchy diagram carries a 720px minimum width), `pre` blocks, and the
 *    `overflow: hidden` on `.preset-card` and `.prop-card` that clips their 3px
 *    accent rail. Content scrolled past those is not dimmed or partly drawn —
 ...
 *    draws its figures — the KEK hierarchy, the request-flow lanes, the wrap
 *    replay — in HTML and CSS grid rather than SVG, so the sibling walk finds
 *    nothing to do beyond the shared top bar's two `aria-hidden` glyphs. It is
 ...
 * rule. This page has two that carry meaning, and both are measured by hand
 * instead — see the note beside `auditControlBoundaries` in `gate.ts`:
 * `.preset-card::before` and `.prop-card::before`, each a 3px full-height rail
 * painted in that card's `--accent` (teal, violet, amber or crimson by
 * `data-tone`). On the preset cards the rail is the only thing distinguishing
 * one scenario card from the next at a glance; on the property cards it is the
 * only per-property mark. Both are SC 1.4.11 graphics, and neither oracle in
 * this repo can see either.
 *
 * SECOND, and more unusually: `body::before` is a FIXED, FULL-VIEWPORT GRID
 * OVERLAY — two 1px `linear-gradient` rules at 48px spacing in
 * `rgba(38,62,78,0.22)` dark and `rgba(60,80,100,0.07)` light, under a
 * `mask-image` that fades them from 0.9 to 0.18 down the page — painted at
 * `z-index: 0` over every glyph on the page. It is not an ancestor of anything,
 * so this walk (which composites ancestors) does not see it, and neither does
 * axe.
 *
 * That gap is real, and in this repo it is CLOSED rather than merely bounded:
 * `contrast/measure.mjs` reads its surfaces out of an actual screenshot, so the
 * overlay is already composited into every background it reports, and
 * `e2e/contrast.spec.ts` gates on those numbers. This walk's ratios are exact
 * for the 47 of every 48 rows and columns the grid does not touch; the pixel
 * oracle covers the rest. Neither reduced motion nor any state this gate drives
 * removes the overlay, so it is present in every measurement either takes.
 */
 ...
 * uses. The two genuinely short-lived stylings here are `fadeUp` (0.55s, on
 * every `.panel`, the header, the hero and the footer quote) and `wrap-in`
 * (0.42s, staggered to 0.42s of delay across the four `.wrap-step` cards) —
 * and under the reduced motion this gate asserts, the stylesheet cancels both
 * with `animation: none`, so nothing transient paints and a scan cannot catch a
 * half-faded panel. Both keyframes run `from { opacity: 0 }` with
 * `animation-fill-mode: both`, which is exactly the shape that strands an
 * element invisible when a reduced-motion block cancels it without restoring an
 * end state; it is safe here only because neither element declares an `opacity`
 * of its own, so cancelling reverts to 1. `expectNotBlank` measures that in
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the hero aside is a
     * `color-mix(in oklab, var(--accent) 6%, transparent)`, and Chromium reports
     * that to `getComputedStyle` unchanged rather than converting it to sRGB. A
     * regex that only understands `rgb()/rgba()` sees `null` for it and the walk
     * then falls through to the wrong backdrop — which fabricates failures
     * (dark-on-light read as dark-on-dark) and, far worse, could hide a real
     * one. This repo has shipped that regex twice: `e2e/border-contrast.spec.ts`
     * still parses colours with one, which is safe only because the two
     * `.seal-input` borders it looks at are plain hex. The 2D canvas is the browser's own colour
     ...
     * page's `body`: a `linear-gradient(180deg, #081016, #071016 50%, #060d13)`
     * runs the full height of a document many screens tall, so text near the top
     ...
     *
     * The radial branch is the load-bearing one here, and it is why axe gives up
     * on this page: `body` carries two `radial-gradient(circle at …)` washes in
     * dark theme, a teal one at `top center` and a violet one at `85% 15%`, and
     * every panel above them is translucent (`--panel` is `rgba(8,18,25,0.88)`
     * dark and `rgba(255,255,255,0.92)` light), so both show through to the text.
     * The hero sits directly in the teal wash, and this lab has already had one
     * real AA failure there that only exists because of it — `--text-dim`
     * measured 4.42:1 against the lit surface where it cleared AA everywhere
     * else. axe filed that node as `incomplete`, which is to say it reported
     * nothing at all.
     */
     ...
     *
     * A driven pass here walks the operations panel, the wrap replay, the
     * scenario presets, the KEK hierarchy, the envelope inspector, the request
     * flow, the audit log, the provider comparison table and the five-card
     * security lab — several hundred elements before any state is built, and
     * more once envelopes accumulate. Without the caches the pass re-reads the
     * same computed styles and rects tens of thousands of times, once per state
     * in a drive with more than twenty of them. Nothing mutates the DOM during
     ...
     * is `.hierarchy-scroll` and `.comparison-table` (`overflow-x: auto`, both
     * really overflowing below 900px), `pre`, and — the one that matters most —
     * `<body>` itself, which carries `overflow-x: hidden`. `.sr-only` is
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="deeper-details">`, which holds the entire security lab
      // and the wire internals — five property cards, ten run buttons and the
      // provider comparison table — and ships shut. The gate opens it by
      ...
      // in a helper called `revealAll()`, before every one of its scans.
      ...
      // not merely inherited. `style.css` defines `.sr-only` in its
      // "Accessibility utilities" block as `position:absolute; width:1px;
      // height:1px; overflow:hidden; clip:rect(0,0,0,0)`, and two of them are on
      // the page from first paint. Each has a 1x1 rect and passes
      // `checkVisibility()` while painting exactly zero pixels, so measuring
      // them reports a ratio — typically around 1.1:1 — for ink the compositor
      // never laid down, in every single scan. (The pixel oracle in
      // `contrast/measure.mjs` gets this right for a different reason: hiding a
      // glyph that was never painted changes no pixels, so it finds nothing to
      // measure and drops the row.) It is deliberately narrow: only a ZERO-AREA
      // clip qualifies, so a partially clipped element is still measured, worst
      // case.
     ...
     *
     * What this lab hides was enumerated rather than assumed, and all of it is
     * decoration whose meaning is carried in text beside it: the arrow glyphs in
     * the request-flow lanes and the hierarchy diagram, and the status marks
     * beside the audit-chain and property verdicts, each of which sits next to
     * the word it illustrates in the same ink on the same surface.
     *
     * Nothing on this page hides a VALUE. No key id, no wrapped DEK, no
     * ciphertext, no audit entry, no property verdict and no refusal message is
     ...
     * gate. This repo has already had the neighbouring bug: `gloss()` in
     * `app.ts` used to put an `aria-label` on a role-less `<abbr>`, which ARIA
     * prohibits and the accessibility tree discards, so the definition reached
     * hover users and nobody else. It is a `title` now, and the `incomplete`
     * assertion in `scan` is what keeps it that way.
     */
     ...
     * how this lab builds its DOM: `app.ts` and every module under `ui/` return
     * `innerHTML` template literals. Interpolating a string ARRAY into one makes
     ...
     *
     * Here it is load-bearing in the harmless direction, and the stylesheet says
     * so out loud: `html { background-color: var(--bg) }` carries the comment
     * "Explicit canvas color: WebKit reports body->canvas propagation as
     * transparent, which breaks automated contrast checks". That is this exact
     * problem, found once already from the other end. `body` sets
     * `min-height: 100vh` rather than `height: 100%`, so its box does span the
     * document today — but `min-height` is one character from `height`, and on a
     * page this many screens deep that edit would put every element below the
     * fold against a white canvas that does not exist, in a theme whose surfaces
     * are all near-black.
     */

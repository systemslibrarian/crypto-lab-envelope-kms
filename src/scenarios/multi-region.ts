...
export async function multiRegionScenario(): Promise<{
  ...
  const envelope = await seal(
    encoder.encode('regional payload'),
    us.keyId,
    encoder.encode('region=us-east-1'),
  );
  const moved = await rewrapEnvelope(envelope, eu.keyId);
  return {
    fromRegionKey: us.keyId,
    toRegionKey: eu.keyId,
    sourceVersion: envelope.kekVersion,
    destVersion: moved.kekVersion,
  };
}
